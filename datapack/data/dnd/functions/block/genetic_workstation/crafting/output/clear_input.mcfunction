...
execute if block ~ ~ ~ barrel{Items:[{Slot:21b,id:"minecraft:carrot_on_a_stick"}]} run scoreboard players set @s dnd.dummy 1
...
execute if entity @s[tag=dnd.cultivator,scores={dnd.dummy=1}] run item replace block ~ ~ ~ container.21 with minecraft:bucket
